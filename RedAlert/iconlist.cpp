	...
	if (text) {
		if (bDoAlloc) {
			...
			if (TextFlags == TPF_TYPE) {
				...
				//	Currently never called. Test well if you use IconList with a font other than
				// TPF_TYPE, 	as the character spacing globals get set weirdly, I've found.
				int iWidthToClipAt = IsScrollActive ? Width : Width - UpGadget.Width;
				//	This call will place '\r's in the string where line breaks should occur.
				Format_Window_String_New(text, iWidthToClipAt, iWidthMax, iHeight, szText, 50);
			}
			...
			while (szToken) {
				while (szNextChar < szToken) {
					//	We expected szToken to begin at szNextChar. Since it doesn't, extra
					// break 	characters must have been removed by strtok as they were
...
int IconListClass::Find(const char *szItemToFind) {
	//	Returns -1 if szItemToFind is not found as the text BEGINNING one of the list entries, else index of
	// item. 	Compare is case-sensitive.
	for (int i = 0; i < List.Count(); i++) {
...
int IconListClass::FindColor(RemapControlType *pColorRemap) {
	//	Returns -1 if no items of specified color are found, else first index. Assumes colorptr == colorptr is a
	// valid equality test.
	for (int i = 0; i < List.Count(); i++) {
