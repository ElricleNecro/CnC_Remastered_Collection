...
//	ajw 9/28/98 - Note about FIXIT_CSII. Changes seem to have been made for Aftermath ("Counterstrike II") that: a)
// were 	bug fixes that should never be rolled back, b) change the nature of the game, at least in multi-player.
// This meant 	that the "Red Alert" executable ( == Counterstrike executable ) could no longer be built. Apparently, at
// the time, 	this was justified, as it was believed that no further patches to the RA executable would ever be
// necessary. 	Given that Denzil's DVD changes and my WOLAPI integration are essentially a patch, we've got a problem.
//	We've decided to level the field and make sure every who gets or patches to the new version of Red Alert, CS,
// AM, (and 	their DVD equivalent(s)) will have the same executable. So we're assuming that all of the FIXIT_CSII
// changes are 	permanent (as, in fact, all prior FIXIT_'s are - makes me wonder why the old non-compiling code has to
// hang around 	forever), and fixing the code so that the assumption "this is an Aftermath game" is no longer hard-coded,
//but can 	change at runtime. (Which is what should have been done when Aftermath was created.) 	<This goes for
// the following three defines as well.>
#define FIXIT_CARRIER	     // Adds Aftermath aircraft carrier
#define FIXIT_PHASETRANSPORT // Adds Aftermath cloaking APC
//	ajw - Discovered that engineer changing fields were specifically left out of aftrmath.ini, thus this has no
// effect. 	Engineer changes (and other game rule changes) are in mplayer.ini, which was loaded before
...
// #define	FIXIT_ENGINEER_CAPTURE	// If building not allied, will still capture if engineer not allied with
// building. #define	FIXIT_HELI_LANDING	// Fixes so new helicopters land at free helipad #define
...
inline ThreatType operator~(ThreatType a) { return static_cast<ThreatType>(~static_cast<int>(a)); }
...
typedef enum PlayerColorType
    : char { PCOLOR_NONE = -1,
	     ...
	     PCOLOR_COUNT,
	     PCOLOR_FIRST = 0,
	     PCOLOR_LAST = PCOLOR_COUNT - 1 } PlayerColorType;

/**********************************************************************
**	This enumerates the remap logic to be applied to an object type when
**	it appears in the construction sidebar.
*/
typedef enum RemapType : unsigned char { REMAP_NONE, REMAP_NORMAL, REMAP_ALTERNATE } RemapType;
...
typedef enum ScenarioPlayerEnum
    : char { SCEN_PLAYER_NONE = -1,
	     ...
	     SCEN_PLAYER_MPLAYER,
	     SCEN_PLAYER_COUNT,
	     SCEN_PLAYER_FIRST = 0 } ScenarioPlayerType;
...
typedef enum ScenarioDirEnum
    : char { SCEN_DIR_NONE = -1,
	     SCEN_DIR_EAST,
	     SCEN_DIR_WEST,
	     SCEN_DIR_COUNT,
	     SCEN_DIR_FIRST = 0 } ScenarioDirType;
...
typedef enum ScenarioVarEnum
    : char { SCEN_VAR_NONE = -1,
	     ...
	     SCEN_VAR_LOSE,
	     SCEN_VAR_FIRST = 0 } ScenarioVarType;
...
typedef enum AircraftType
    : char { AIRCRAFT_TRANSPORT, // Transport helicopter.
...
*/
typedef enum DoType
    : char { DO_NOTHING = -1, // Not performing any choreographed sequence.
	     DO_STAND_READY = 0,
	     DO_STAND_GUARD,
	     DO_PRONE,
	     DO_WALK,
...
inline TextPrintType operator~(TextPrintType a) { return static_cast<TextPrintType>(~static_cast<int>(a)); }
...
typedef enum WindowNumberType : unsigned char {
	...
	WINDOW_VIRTUAL // Window that is used when we want to capture rendering information without actually rendering.
		       // ST - 1/15/2019 3:03PM
} WindowNumberType;
...
typedef enum FacingType
    : char { FACING_NONE = -1,
	     ...
	     FACING_FIRST = 0 } FacingType;

// inline FacingType operator++(FacingType &, int);
inline FacingType operator+(FacingType f1, FacingType f2) { return (FacingType)(((int)f1 + (int)f2) & 0x07); }
inline FacingType operator+(FacingType f1, int f2) { return (FacingType)(((int)f1 + (int)f2) & 0x07); }
...
inline int operator*(FacingType f1, FacingType f2) { return ((int)f1 * (int)f2); }
...
typedef enum SpeedType
    : char { SPEED_NONE = -1,
	     ...
	     SPEED_COUNT,
	     SPEED_FIRST = SPEED_FOOT } SpeedType;
...
typedef enum SourceType
    : char { SOURCE_NONE = -1, // No defined source (error condition).
...
*/
typedef enum : unsigned char { SDLG_NONE, SDLG_OPTIONS, SDLG_SURRENDER, SDLG_SPECIAL } SpecialDialogType;

typedef enum : unsigned char { CC_MOUSE_COLOR = 16 } CCPaletteType;
...
typedef enum OptionControlType
    : char { OPTION_NONE = -1,	       // No fancy shmancy shape.
	     ...
	     OPTION_COUNT } OptionControlType;
