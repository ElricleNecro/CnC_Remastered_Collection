 ...
 ** This class has to set up an IPX Event Service Routine in low (DOS)		* memory.  So, it uses DPMI to allocate
 *& lock a chunk of DOS memory;		* this memory is used for all incoming packet buffers, the outgoing
 ** packet buffer, and the actual code for the event handler.  The real-		* mode handler code & this class
 *share a portion of memory that's mapped	* into a "RealModeDataType" structure.  As packets come in, the handler
 ** points IPX to the next available packet buffer & restarts listening;		* it sets a flag to tell this
 *class that a packet is present at that		* buffer slot.  This class must read all the packets & determine
 *which		* connection they go with (the Global Channel, or one of the Private		* Channels).  This
 *parsing is done in the Service routine for this class.	*
 * 																								*
 ...
 *						to occur.
 ** Connection utilities: Create & manage Private Connections.  Each 			* connection has its own IPX
 *address, numerical ID, and		* character name (presumably the name of the other			*
 *						player).
 ** Send/Get_Global_Message: adds a packet to the Global Connection queue,	* or reads from the queue.  The caller
 *should check the		* ProductID value from returned packets to be sure it's		* talking to the right
 *product.										* Send/Get_Private_Message: adds
 *a packet to a Private Connection queue,	* or reads from the queue
 ** Service:			Checks the Real-Mode-Memory packet array to see if any	* new packets have come in; if
 *they have, it parses them	* & distributes them to the right connection queue.  The	* queue's Service
 *routine handles ACK'ing or Resending		* packets.
 **
 *																									*
