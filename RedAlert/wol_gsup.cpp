...
WOL_GameSetupDialog::WOL_GameSetupDialog(WolapiObject *pWO, bool bHost)
    : pWO(pWO), bHost(bHost), HousePrevious(HOUSE_NONE), pILPlayers(NULL), pILScens(NULL), pILDisc(NULL),
      pEditSend(NULL), pGaugeCount(NULL), pGaugeLevel(NULL), pGaugeCredits(NULL), pGaugeAIPlayers(NULL),
      pCheckListOptions(NULL),
      //		pTextBtnOk( NULL ),
      pTextBtnCancel(NULL), pTextBtnAcceptStart(NULL), pTextBtnAction(NULL), pStaticDescrip(NULL), pStaticUnit(NULL),
      pStaticLevel(NULL), pStaticCredits(NULL), pStaticAIPlayers(NULL), pDropListHouse(NULL),
      pCheckAftermathUnits(NULL), nHostLastParamID(0), nGuestLastParamID(0), bWaitingToStart(false),
      bParamsReceived(false), bHostSayGo(false), bExitForGameTrigger(false), pToolTipHead(NULL), pToolTipHitLast(NULL),
      pTTipAcceptStart(NULL), pTTipCancel(NULL), pTTipAction(NULL), ScenKindCurrent(SCENARIO_UNINITIALIZED),
      pShpBtnScenarioRA(NULL), pShpBtnScenarioCS(NULL), pShpBtnScenarioAM(NULL), pShpBtnScenarioUser(NULL),
      bLeaveDueToRulesMismatchTrigger(false), bHostWaitingForGoTrigger(false) {
	*szSendBuffer = 0;
	*szHouseBuffer = 0;
...
void WOL_GameSetupDialog::Initialize() {
	...
	*/
	...
	pILPlayers =
	    new IconListClass(BUTTON_PLAYERLIST, d_playerlist_x, d_playerlist_y, d_playerlist_w, d_playerlist_h,
			      TPF_TYPE, MFCD::Retrieve("BTN-UP.SHP"), MFCD::Retrieve("BTN-DN.SHP"), true, 2);
	...
	pILScens = new IconListClass(BUTTON_SCENARIOLIST, d_scenariolist_x, d_scenariolist_y, d_scenariolist_w,
				     d_scenariolist_h, TPF_TYPE, MFCD::Retrieve("BTN-UP.SHP"),
				     MFCD::Retrieve("BTN-DN.SHP"), true, 1);
	pILDisc = new IconListClass(BUTTON_DISCLIST, d_disc_x, d_disc_y, d_disc_w, d_disc_h, TPF_TYPE,
				    MFCD::Retrieve("BTN-UP.SHP"), MFCD::Retrieve("BTN-DN.SHP"), true, 0, 300);

	pEditSend = new EditClass(BUTTON_SENDEDIT, szSendBuffer, MAXCHATSENDLENGTH, TPF_TEXT, d_send_x, d_send_y,
				  d_send_w, d_send_h);
	...
	pCheckListOptions = new CheckListClass(BUTTON_PARAMS, d_options_x, d_options_y, d_options_w, d_options_h,
	...
	if (bHost) {
		pTextBtnAcceptStart = new TextButtonClass(BUTTON_ACCEPTSTART, TXT_WOL_STARTBUTTON, TPF_BUTTON,
							  d_accept_x, d_accept_y, d_accept_w);
		pTTipAcceptStart = new ToolTipClass(pTextBtnAcceptStart, TXT_WOL_TTIP_START,
						    d_accept_x + d_accept_w / 2, d_accept_y - 6);
	} else {
		pTextBtnAcceptStart = new TextButtonClass(BUTTON_ACCEPTSTART, TXT_WOL_ACCEPTBUTTON, TPF_BUTTON,
							  d_accept_x, d_accept_y, d_accept_w);
		pTTipAcceptStart = new ToolTipClass(pTextBtnAcceptStart, TXT_WOL_TTIP_ACCEPT,
						    d_accept_x + d_accept_w / 2, d_accept_y - 6);
	}

	pTextBtnAction =
	    new TextButtonClass(BUTTON_ACTION, TXT_WOL_ACTION, TPF_BUTTON, d_action_x, d_action_y, d_action_w);
	pTTipAction =
	...
	//	pStaticDescrip is no longer used - can't get the bloody thing to clip text. You'd think a StaticButton
	// control would.
	//	pStaticDescrip = new StaticButtonClass( 0, "", TPF_TYPE, d_gamekind_x, d_gamekind_y, d_gamekind_w,
	// d_gamekind_h );
	...
	pDropListHouse =
	    new DropListClass(BUTTON_HOUSE, szHouseBuffer, sizeof(szHouseBuffer), TPF_TEXT, d_house_x, d_house_y,
			      d_house_w, d_house_h, MFCD::Retrieve("BTN-UP.SHP"), MFCD::Retrieve("BTN-DN.SHP"));

	//	ajw - This checkbox is not used. Could be turned on, though.
	pCheckAftermathUnits = new BigCheckBoxClass(BUTTON_AFTERMATHUNITS, d_amunits_x, d_amunits_y, d_amunits_w,
						    d_amunits_h, "Aftermath units enabled", TPF_TEXT, false);
...
#define TABSPACING 38
	pShpBtnScenarioRA = new ShapeButtonClass(BUTTON_SCENARIO_RA, MFCD::Retrieve("tabra.shp"), d_scenariolist_x,
						 d_scenariolist_y - d_tab_h);
	pShpBtnScenarioCS = new ShapeButtonClass(BUTTON_SCENARIO_CS, MFCD::Retrieve("tabcs.shp"),
						 d_scenariolist_x + TABSPACING, d_scenariolist_y - d_tab_h);
	pShpBtnScenarioAM = new ShapeButtonClass(BUTTON_SCENARIO_AM, MFCD::Retrieve("tabam.shp"),
						 d_scenariolist_x + TABSPACING, d_scenariolist_y - d_tab_h);
	...
	pShpBtnScenarioUser = new ShapeButtonClass(BUTTON_SCENARIO_USER, MFCD::Retrieve("tabus.shp"),
	...
	if (!bParamsReceived) {
		...
		if (bHost) {
			...
			for (i = 0; i < Session.Scenarios.Count(); i++) {
				//	Reworking of the loop previously used for language translation. (What a hack I
				// have inherited...)
				...
				if (pMMission->Get_Official()) {
					if (Is_Mission_Counterstrike((char *)(Session.Scenarios[i]->Get_Filename()))) {
						...
						// an Aftermath game channel, we must filter out any AM maps that have
		...
		//				debugprint( "Illegal GameInfoCurrent value." );
			...
			*/
			...
			ScenarioDisplayMode(SCENARIO_RA); //	Always start on RedAlert tab. Next line depends on
							  // selected item in 	list matching selected scenario.
			//			pStaticDescrip->Set_Text( pILScens->Get_Item( pILScens->Current_Index()
			//), false );
	...
	while (bProcess) {
		...
		if (HousePrevious != Session.House) {
			if (bHost) {
				...
				if (pUserHost) //	Else we have not received the user list yet and don't know who
					       // the host is. 	We'll keep trying this until we get a host -
					       // HousePrevious keeps us triggering until then.
				{
					...
					//	Set house in our own list. This is fine because we know that the change
					// must be affirmed by the host.
					SetPlayerHouse(pWO->szMyName, Session.House);
					HousePrevious = Session.House;
				}
			}
		...
		//	Display of ping results is on a parallel timer, slightly behind the ping so that we'll be likely
		// to have 	a fresh average to show. Not too big a deal if OnPings haven't arrived by then (though
		// they should have).
		...
		if (::timeGetTime() > pWO->dwTimeNextWolapiPump) {
			...
			if (pWO->pChatSink->bGotKickedTrigger) {
				...
				//	Leave the bGotKickedTrigger flag so we can react to it upon reentering the chat
				// dialog. pWO->pChatSink->bGotKickedTrigger = false;
				display = REDRAW_ALL;
			}
		...
		if (bHostSayGo) {
			...
			//			debugprint( "Host about to exit game setup dialog for game.\n" );
						if( !ExitGameChannel() )
							ResultReturn = RESULT_WOLGSUP_FATALERROR;	//	Return
			with an error value. break;
			*/
		} else if (*szNameOfHostWhoJustBailedOnUs) //	Host left channel - cancel setup.
		{
			//			debugprint( "Guest about to exit game setup dialog because host bailed
			// on us.\n" );
			if (ExitGameChannel()) {
				pWO->RejoinLobbyAfterGame();
				bProcess = false;
				ResultReturn = RESULT_WOLGSUP_HOSTLEFT; //	Return to chat.
				//	Add a message explaining what happened to the saved chat that will be restored
				// in the chat dialog.
			...
			//			debugprint( "Guest about to exit game setup dialog because of rules.ini
			// mismatch.\n" );
			if (ExitGameChannel()) {
				pWO->RejoinLobbyAfterGame();
				bProcess = false;
		...
		if (display) {
			...
			*/
			...
			if (display >= REDRAW_BACKGROUND) {
				...
				if (bHost)
					Fancy_Text_Print(TXT_SCENARIOS, d_scenariolist_x + d_scenariolist_w,
							 d_scenariolist_y - 12, scheme, TBLACK, TPF_TYPE | TPF_RIGHT);
				//				else
				//					Fancy_Text_Print( TXT_SCENARIO_COLON,
				// d_scenariolist_x + (d_scenariolist_w / 2), d_scenariolist_y - d_txt6_h, scheme,
				...
				Fancy_Text_Print(TXT_COLOR_COLON, d_color_x + d_color_w * 4, d_color_y - d_txt6_h,
				...
				case CREATEGAMEINFO::AMGAME:
					...
					break;
				default:
					//					debugprint( "Illegal GameInfoCurrent
					// value." ); Fatal( "Illegal GameInfoCurrent value." );
					szGameKind = TXT_WOL_CG_AMGAME;
					pDIB = NULL;
				...
				//	Game kind.
				Fancy_Text_Print(szGameKind, d_gamekind_x, d_gamekind_y - iGameInfoSpacingY * 1, scheme,
						 TBLACK, TPF_TYPE);
				//	Game kind icon.
				CC_Draw_DIB(pDIB, d_gamekind_x - 16, d_gamekind_y - iGameInfoSpacingY * 1 - 2, 100,
					    WINDOW_MAIN);
				//	"Tournament."
				...
				if (pWO->GameInfoCurrent.bPrivate) {
					...
					Fancy_Text_Print(szPrivatePassword, d_gamekind_x + iGameInfoSecondColumnX,
							 d_gamekind_y + iGameInfoSpacingY * 2, scheme, TBLACK,
							 TPF_TYPE);
				...
				// Fancy_Text_Print( TXT_SCENARIO_COLON, d_gamekind_x, d_gamekind_y - iGameInfoSpacingY,
				// scheme, TBLACK, TPF_TYPE );
			}
			...
			if (display >= REDRAW_MESSAGE) {
				//				Draw_Box(d_disc_x, d_disc_y, d_disc_w, d_disc_h,
				// BOXSTYLE_BOX, true); 				Draw_Box(d_send_x, d_send_y,
				// d_send_w, d_send_h, BOXSTYLE_BOX, true);
			...
			if (display >= REDRAW_PARMS) {
				...
				if (!bHost) {
					...
					szScenarioFileName =
					    (char *)Session.Scenarios[Session.Options.ScenarioIndex]->Get_Filename();
				}
				...
				//	Print scenario name.
				Conquer_Clip_Text_Print(szScenarioNameDisplay, d_gamekind_x, d_gamekind_y, scheme,
							TBLACK, TPF_TYPE, d_gamekind_w);
			...
			if (!bWaitingToStart) {
				...
				if (!bHost) {
					if ((Get_Mouse_X() >= d_count_x && Get_Mouse_X() <= d_count_x + d_count_w &&
						...
						// *)Text_String(TXT_ONLY_HOST_CAN_MODIFY), PCOLOR_BROWN, TPF_TEXT,
						// 1200);
				...
				if (Keyboard->MouseQX > cbox_x[0] &&
					...
					if (pILPlayers->FindColor(&ColorRemaps[Session.PrefColor == PCOLOR_DIALOG_BLUE
										   ? PCOLOR_REALLY_BLUE
										   : Session.PrefColor]) == -1) {
		...
		case (BUTTON_HOUSE | KN_BUTTON):
			...
			/*
							//	Bloody bloody hell I can't believe there are bugs in RA
			like the one I deal with here... if( strcmp( pDropListHouse->Current_Item(), "Russia" ) == 0 )
								Session.House = HOUSE_USSR;
							else
							{
								Session.House = HouseTypeClass::From_Name(
			pDropListHouse->Current_Item() );	//	Fails on "Russia". (Thinks "USSR".) if(
			Session.House == HOUSE_NONE )
								{
			//						debugprint( "Couldn't find house from selected
			...
			if (pILScens->Count()) {
				...
				if (iSelectedScenIndex != Session.Options.ScenarioIndex) {
					...
					if (!pILScens->SetSelectType(
						1)) //	Hack to deal with ListClass "highlighting nothing" problem.
		...
		case (BUTTON_COUNT | KN_BUTTON):
			Session.Options.UnitCount =
			    pGaugeCount->Get_Value() + SessionClass::CountMin[Session.Options.Bases];
			bInformParamChange = true;
			if (display < REDRAW_PARMS)
		...
		case (BUTTON_ACCEPTSTART | KN_BUTTON): //	'Accept' or 'Start Game' button.
			if (!bHost) {
				...
				{
					//	Set ourself as accepted. We want to do this immediately so the user has
					// feedback for pressing button. 	Besides, the host is guaranteed to allow
					// this and set us to "accepted", unless it is simultaneously 	sending us a new
					// update we haven't received yet. If this is the case, we will get that update
					// in a 	second and will set ourselves to unaccepted, which will match the
					// unaccepted state on the server. 	Upshot - We can ignore messages from the
					// server telling us that we, ourself, accepted.
					if (pToolTipHitLast && pToolTipHitLast->bShowing)
						pToolTipHitLast->Unshow();
					pTextBtnAcceptStart->Disable();
					if (SetPlayerAccepted(pWO->szMyName,
				...
				//	If we have changes just made and not yet sent, don't say start, because we're
				// about to send changes 	that will unaccept everyone.
				if (!bParamsUnfresh()) {
					//	Force user to put the correct disk in before proceeding. (Not crucial,
					// but can lead to ugly 	timeouts if the scenario has to be downloaded
					// before game start.)
					if (!Session.Scenarios[Session.Options.ScenarioIndex]->Get_Official() ||
						...
						//	Go into "waiting to start" mode, tell guests to, and wait for
						// responses.
...
void WOL_GameSetupDialog::BindControls(bool bBind) {
	if (bBind) {
		...
		if (bHost) {
			//	Draw order of tabs depends on which one is selected, as we want them to overlap
			// appropriately. 	Also, the selected tab must appear over the scenario list, while the
			// unselected tabs are below it. 	This assures that that bottom of the tab overlaps the
			// scenario list border, making it look connected.

			//	ajw I could make all maps always available now that they're downloadable. Playing CS map
			// with AM rules 	would mean switching CDs, though. 	Would mean no difference between
			// RA and CS games.
			...
			case SCENARIO_CS: //	pWO->GameInfoCurrent.GameKind must be CREATEGAMEINFO::CSGAME.
				...
				break;
			case SCENARIO_AM: //	pWO->GameInfoCurrent.GameKind must be CREATEGAMEINFO::AMGAME, or RAGAME
					  // with AM installed.
				if (!pWO->GameInfoCurrent.bTournament) {
					pShpBtnScenarioUser->Add_Tail(*commands);
					pShpBtnScenarioUser->DRAWTABDOWN;
...
void WOL_GameSetupDialog::ScenarioDisplayMode(SCENARIO_GAMEKIND ScenKind) {
	...
	//	Reset list items.
	pILScens->Clear();
	//	debugprint( "  *  *  *  *  *  *  ScenKind %i has %i items.\n", ScenKind, ar_szScenarios[ ScenKind
	//].Count() ); 	Check for the currently selected scenario.
	...
	for (int i = 0; i != ar_szScenarios[ScenKind].Count(); i++) {
		...
		int iIndexNew =
		    pILScens->Add_Item(ar_szScenarios[ScenKind][i], NULL, NULL, ICON_DIB, NULL, (void *)iScenIndex);
...
void WOL_GameSetupDialog::ProcessGuestRequest(User *pUser, const char *szRequest) {
	...
	case WOL_GAMEOPT_REQCOLOR: {
		...
		if (pILPlayers->FindColor(
			&ColorRemaps[ColorDesired == PCOLOR_DIALOG_BLUE ? PCOLOR_REALLY_BLUE : ColorDesired]) == -1) {
			//	Color is available.
	...
	case WOL_GAMEOPT_REQACCEPT:
		...
		if (atoi(szRequest) == nHostLastParamID) {
			...
			//			debugprint( "______________Host received invalid accept from '%s'. ID =
			//%i when it should be %i.\n", (char*)pUser->name, 				atoi( szRequest
			//), nHostLastParamID );
		}
		break;
	case WOL_GAMEOPT_REQSTART:
		//	Does Param ID of accept request match the last param change ID sent? See notes at top.
		if (atoi(szRequest) == nHostLastParamID) //	Otherwise ignore - it's old and we don't care.
							 //(Incredibly unlikely to happen, actually.)
		{
			//			debugprint( "Host received valid WOL_GAMEOPT_REQSTART from '%s'.\n",
			//(char*)pUser->name ); 			WOL_PrintMessage( *pILDisc,
			...
			//			else
			//				debugprint( "Ignoring - I am no longer waiting to start a
			// game.\n" );
		}
		break;
	case WOL_GAMEOPT_REQSTART_BUTNEEDSCENARIO:
		...
		if (atoi(szRequest) == nHostLastParamID) //	Otherwise ignore - it's old and we don't care.
							 //(Incredibly unlikely to happen, actually.)
		{
			//			debugprint( "Host received valid WOL_GAMEOPT_REQSTART_BUTNEEDSCENARIO
			// from '%s'.\n", (char*)pUser->name );
			if (bWaitingToStart)
				//	If all responses are in, start the game!
	...
	case WOL_GAMEOPT_INFGO:
		...
		//	This is the new method. Avoids apparent "private messages/gameopts are getting delayed" problem
		// in chatserver... 	(I don't want to end up leaving the channel before guests get my go message.)
...
void WOL_GameSetupDialog::ProcessInform(char *szInform) {
	...
	if (!bHost) {
		...
		case WOL_GAMEOPT_INFCOLOR: {
			...
			szInform += 3;
			SetPlayerColor(szInform,
				       Color); //	(szInform is now sitting at the start of the name string.)
			Sound_Effect(VOC_OPTIONS_CHANGED);
			break;
		}
		case WOL_GAMEOPT_INFHOUSE: //	Note: In theory, I could ignore this if it refers to me. I've already
					   // set my own house.
		{
			...
			szInform += 3;
			SetPlayerHouse(szInform,
				       House); //	(szInform is now sitting at the start of the name string.)
			ClearAllAccepts();
			Sound_Effect(VOC_OPTIONS_CHANGED);
			break;
		}
		case WOL_GAMEOPT_INFACCEPT: //	Note: In theory, I could ignore this if it refers to me.
			...
			SetPlayerAccepted(szInform,
					  true); //	(szInform is now sitting at the start of the name string.)
			break;
		...
		case WOL_GAMEOPT_INFSTART: {
			...
			nGuestLastParamID = atoi(szInform);
			//	The following check is not necessary. Rules.ini, if manually replaced by a cheater, is
			// not reloaded. 	So prior checks (that occur on game params receives) are sufficient.
			// szInform += 7;
			...
			if (pUserHost) //	This better'd be true.
			{
				...
				WWMessageBox().Process(TXT_WOL_WAITINGTOSTART, TXT_NONE);
				BindControls(false);
				//	If we are in a modal dialog, we must have arrived here through Call_Back()'s
				// PumpMessages. 	Set global that will force edit dialogs to stop accepting
				// characters. 	This is to fix a minor glitch: guests can keep typing into a "page"
				//dialog editbox after the 	"Launching game..." message has appeared on top of it.
		...
		case WOL_GAMEOPT_INFCANCELSTART:
			...
			display = REDRAW_ALL;
			//	If we are in a modal dialog, we must have arrived here through Call_Back()'s
			// PumpMessages. Set global that will 	force a cancel out of the dialog.
			if (pWO->bPump_In_Call_Back)
		...
		default:
			...
			// WOL_PrintMessage( *pILDisc, "Error - Unhandled value in ProcessInform!!!",
			// WOLCOLORREMAP_LOCALMACHINEMESS );
...
void WOL_GameSetupDialog::SendParams() {
	...
	sprintf(szSend,
		...
		"%u "
		"%u ",
		WOL_GAMEOPT_INFPARAMS, nHostLastParamID, strlen(GParamsLastSent.GPacket.ScenarioInfo.Scenario),
		GParamsLastSent.GPacket.ScenarioInfo.Scenario, GParamsLastSent.GPacket.ScenarioInfo.FileLength,
		GParamsLastSent.GPacket.ScenarioInfo.ShortFileName,
		//		strlen( (char*)GParamsLastSent.GPacket.ScenarioInfo.FileDigest ),		not
		// null-terminated!
		GParamsLastSent.GPacket.ScenarioInfo.FileDigest[0] ? 1 : 0,
		GParamsLastSent.GPacket.ScenarioInfo.FileDigest, GParamsLastSent.GPacket.ScenarioInfo.OfficialScenario,
		GParamsLastSent.GPacket.ScenarioInfo.Credits, GParamsLastSent.GPacket.ScenarioInfo.IsBases,
		GParamsLastSent.GPacket.ScenarioInfo.IsTiberium, GParamsLastSent.GPacket.ScenarioInfo.IsGoodies,
		GParamsLastSent.GPacket.ScenarioInfo.BuildLevel, GParamsLastSent.GPacket.ScenarioInfo.UnitCount,
		GParamsLastSent.GPacket.ScenarioInfo.AIPlayers, GParamsLastSent.GPacket.ScenarioInfo.Seed,
		...
		GParamsLastSent.GPacket.ScenarioInfo.Special.IsTSpread, GParamsLastSent.GPacket.ScenarioInfo.GameSpeed,
...
bool WOL_GameSetupDialog::AcceptParams(char *szParams) {
	...
	if (iLen) {
		...
		//		//	//	Null-terminate.
		//		//	Session.ScenarioDigest[ iLen ] = 0;			Digest has no
		// null-terminator!
		//		//	Advance string pointer to next param.
		//		szRemaining += iLen + 1;
		//	There is a digest.
...
bool operator==(const GlobalPacketType &gp1, const GlobalPacketType &gp2) {
	...
	if (strcmp(gp1.ScenarioInfo.ShortFileName, gp2.ScenarioInfo.ShortFileName) != 0)
		return false;
	//	Digest is not null-terminated...
	if (strncmp((const char *)gp1.ScenarioInfo.FileDigest, (const char *)gp2.ScenarioInfo.FileDigest,
...
void WOL_GameSetupDialog::OnGuestJoin(User *pUser) {
	...
	if (bHost) {
		...
		//	Previously, I was sending an individual color, house, and acceptedstate message for each other
		// guest. 	Though convenient code-wise, this causes the initial info to arrive at the new guest in
		// a very slow 	manner. This is because of the wonderful "anti-flood" feature of the chat server, which
		// prevents a 	series of messages from a client from being passed on faster than a certain rate.
		...
		for (int i = 0; i < pILPlayers->Count(); i++) {
			...
			//			InformAboutPlayerColor( szPlayerName, PlayerColorTypeOf(
			// pILPlayers->Get_Item_Color( i ) ), pUser );
			...
			if (strcmp(szPlayerName, (char *)pUser->name) != 0) {
				...
				if (House != HOUSE_NONE) {
					//				InformAboutPlayerHouse( szPlayerName, House,
					// pUser );
					char szSendHouse[50];
					sprintf(szSendHouse, " 1 %02u", (short)House);
					strcat(szSendPiece, szSendHouse);
				} else {
					//	Player must not have told me what house he is yet. Don't send house
					// value.
			...
			//			//	Send "accepted" status of player. Ignore myself, as I'm the
			// host. 			if( strcmp( szPlayerName, pWO->szMyName ) != 0 &&
			// pWO->bItemMarkedAccepted( i ) ) 				strcat( szSendPiece, " 1" ); 			else 				strcat( szSendPiece, " 0"
			//);
		...
		//	Send game params.
		//	This is done last because it contains a param ID value, and we need to ensure that the new guest
		// has 	received everything we are sending him here before he tries to send me an accept. 	If game
		// params were sent first, he could theoretically receive them, then send an accept, even though he
		// has not received the WOL_GAMEOPT_INFNEWGUESTPLAYERINFO. 	By doing this I avoid having to have a
		// param ID in WOL_GAMEOPT_INFNEWGUESTPLAYERINFO, which would be hard, 	since it is a private message.
...
void WOL_GameSetupDialog::AcceptNewGuestPlayerInfo(char *szMsg) {
	...
	for (unsigned int nPlayer = 0; nPlayer != nPlayers; ++nPlayer) {
		...
		if (bHouseField) {
			...
			//	SetPlayerHouse may call strtok, so we can't use the strtok( NULL, option... in the next
			// call.
			szRemaining += 5;
		} else
...
void WOL_GameSetupDialog::ClearAllAccepts() {
	...
	for (int i = 0; i < pILPlayers->Count(); i++) {
		...
		if (pUser &&
		    !(pUser->flags &
		      CHAT_USER_CHANNELOWNER)) //	pUser null if this is an "early insertion" entry on startup
	...
	if (!bHost) {
		if (Find_Local_Scenario(Session.Options.ScenarioDescription, Session.ScenarioFileName,
			...
			*/
			...
			// be happening with am maps when i have am...?
...
void WOL_GameSetupDialog::HostSaysGo() {
	...
	while (pUser) {
		...
		sprintf(szUser, " %s %02u", (char *)pUser->name,
			Color); //	What if player left just now, and got removed from list. Ok to continue and fail
				// on game start?
	...
	/*	...Method changed. It appears that my channelleave may appear to guests before the "go" privategameopt.
		...
		//	(ajw - Note: An apparent bug in wolapi means that this pump does not necessarily flush the go
	   gameopts.
...
void WOL_GameSetupDialog::TriggerGameStart(char *szGoMessage) {
	...
	//	WWGetPrivateProfileString("Options", "Handle", "Noname", Session.Handle, sizeof(Session.Handle),
	// buffer);
	...
	//	gotit	Session.Options.AIPlayers = WWGetPrivateProfileInt("Options", "AI", 0, buffer);		//Number
	// of AI players
	...
	//	Now do whatever we've left out that the horrific Net_Fake_New_Dialog() and Net_Fake_Join_Dialog() used
	// to do for us...
	...
	//	I will set "ID" based on a player's color, though it seems unclear if this is even used in the game, or
	// what it should be.

	Clear_Vector(&Session.Players);

	//	Make the pILPlayers a valid list of players in the game.
	//	Players might (incredibly rarely) have joined in the last split-second, and we only want the players
	// listed in 	the szGoMessage. To test for whether they're in this list, first wipe the colors from all list
	// items. 	Then we fill them in from info in szGoMessage. 	We can ignore any list items then that have no
	// color assigned. 	Also, we'll know that the colors assigned to valid players indeed match up with what
	// every other client has. 	Remember, all other data should already be sync'ed because it has been
	// implemented in such a way that changes would 	cause "unacceptedness" of guests to occur.
	...
	for (iItem = 0; iItem < pILPlayers->Count(); iItem++) {
		//	The following is not very efficient, but doesn't have to be. Better in this case to keep it
		// clear and simple.
		...
		if (strcmp(szPlayerName, pWO->szMyName) != 0 && GetPlayerColor(szPlayerName) != PCOLOR_NONE) {
			...
			if (pWO->GameInfoCurrent.bTournament) {
				...
				//	for convenience, save a copy of his IP address in case I need it later for
				// disconnect pinging.
			...
			//	If player is the game host, set HostAddress. This global is used when downloading
			// scenarios; who knows where else.
			...
			if (pUser && pUser->flags & CHAT_USER_CHANNELOWNER) {
				...
				nodexxx[1], nodexxx[2], nodexxx[3], nodexxx[4], nodexxx[5] );
				*/
			}
			/*
						else
	...
	*/
	...
	//										sizeof
	//(Session.Options.ScenarioDescription),
	//buffer); WWDebugString ("RA95I - Scenario is "); WWDebugString
	// (Session.Options.ScenarioDescription); WWDebugString ("\n");
	...
	if (!bHost) //	Else ScenarioIndex is already set.
	{
		if (bRequestedScenarioDownload) {
			...
			if (bSpecialAftermathScenario(Session.Options.ScenarioDescription)) {
				//	Shouldn't ever happen. We should never have the opportunity to ask for one of
				// these maps to be downloaded.
			...
			/*	This is how the same code existed previously. Insufficient because Description may match
			   on many scenarios. Session.Options.ScenarioIndex = -1; for (int i = 0; i <
			   Session.Scenarios.Count(); i++) { if (!strcmp (Session.Scenarios[i]->Description(),
			   Session.Options.ScenarioDescription) ){ Session.Options.ScenarioIndex = i; break;
							}
						}
			*/
