...
STDMETHODIMP RAChatEventSink::OnLogout(HRESULT hRes, User *pUser) {
	...
	if (hRes == S_OK) {
		...
		// pUser->flags & CHAT_USER_CHANNELOWNER ) );
...
STDMETHODIMP RAChatEventSink::OnChannelJoin(HRESULT hRes, Channel * /*pChannel*/, User *pUser) {
	//	if( SUCCEEDED( hRes ) )
	//		debugprint( ">>> OnChannelJoin got: channel '%s', user '%s', %i ", (char*)pChannel->name,
	//(char*)pUser->name, hRes ); 	else 		debugprint( ">>> OnChannelJoin got: %i ", hRes );
	DebugChatDef(hRes);
	...
	if (SUCCEEDED(hRes)) {
		if (pUser->flags & CHAT_USER_MYSELF) {
			...
			if (pUserList) {
				//	Should never happen.
				//				debugprint( "pUserList should be NULL (as I am the
				// joiner)!!! Deleting user list...\n" );
				DeleteUserList();
			}
		} else {
			...
			if (!pUserList) {
				//				debugprint( "pUserList is null in OnChannelJoin -
				// ignoring %s join... \n", (char*)pUser->name );
				return S_OK;
			}
			//			if( !pUserList )
...
STDMETHODIMP RAChatEventSink::OnChannelLeave(HRESULT hRes, Channel *, User *pUser) {
	...
	if (SUCCEEDED(hRes)) {
		if (pUser->flags & CHAT_USER_MYSELF) {
			...
			if (!pUserList) {
				//				debugprint( "pUserList is null in OnChannelLeave -
				// ignoring %s leave... \n", (char*)pUser->name );
				return S_OK;
			}
			if (pOwner->CurrentLevel == WOL_LEVEL_INGAMECHANNEL) {
				...
				if (_stricmp((char *)pUserSearch->name, (char *)pUser->name) == 0) {
					...
					if (!pUserPrevious) {
						...
						if (!pUserList) {
							...
							Fatal("This means all entries were removed. Can't happen, as "
							      "you are still there. (OnChannelLeave)\n");
						}
					} else {
			...
			if (!bFound) {
				...
				//				debugprint( "User not found for removal in
				// OnChannelLeave!!!\n" );
				return S_OK;
			}
...
STDMETHODIMP RAChatEventSink::OnChannelList(HRESULT, Channel *pChannelListIn) {
	...
	{
		//		debugprint( ">>> IGNORED OnChannelList, filter = %i, WO's LastUpdateChannelCallLevel =
		//%i \n", ChannelFilter, pOwner->LastUpdateChannelCallLevel );
		return S_OK;
	}

	//	Special case for modal GetLobbyChannels(). Because we want to be sure this OnChannelList is one that was
	// caused 	by a Request for gametype 0, and not one arriving from an earlier Request for games. 	This
	// OnChannelList might not actually match the Request in GetLobbyChannels(), but as long as it's type 0 it'll
	// do.
	if (bRequestChannelListForLobbiesWait) {
		...
		//	(In our case assume it was the one asking for lobbies and allow to fail later naturally due to
...
bool RAChatEventSink::DownloadUpdates(Update *pUpdateList, int iUpdates) {
	...
	while (pUpdate) {
		...
		//	Make sure the destination directory exists, and make it the current directory during the
		// download.
...
STDMETHODIMP RAChatEventSink::OnUserIP(HRESULT hRes, User *pUser) {
	//	A list of users is kept, separate from other user lists, to preserve the ipaddr's we've found through
	// this 	callback. OnUserList (for some dumb reason) doesn't hold valid ipaddr's, so we have to go
	// through 	all this rigamarole... 	(List is cleared when entering game channel. Users are added initially
	// and on joins, not removed on leaves.) 	debugprint( ">>> OnUserIP got: " );
	DebugChatDef(hRes);
...
STDMETHODIMP RAChatEventSink::OnUserFlags(HRESULT hRes, LPCSTR name, unsigned int flags, unsigned int) {
	...
	if (pOwner->CurrentLevel == WOL_LEVEL_INGAMECHANNEL) {
		if (pOwner->pGSupDlg && (pOwner->pGSupDlg->bHostSayGo || pOwner->pGSupDlg->bHostWaitingForGoTrigger ||
					 pOwner->pGSupDlg->bExitForGameTrigger || iGameID)) {
			//	A game has this moment entered the "must start" phase. We must ignore the fact that
			// others are leaving the channel.
