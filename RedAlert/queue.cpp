 ...
 *                                                                         *
 * Functions for processing Queued Events: * Queue_AI -- Process all queued events.                                *
 *   Queue_AI_Normal -- Process all queued events.                         *
 *   Queue_AI_Multiplayer -- Process all queued events.                    *
 *                                                                         *
 * Main Multiplayer Queue Logic:
 ** Wait_For_Players -- Waits for other systems to come on-line           * Generate_Timing_Event -- computes & queues a
 *RESPONSE_TIME event      * Process_Send_Period -- timing for sending packets every 'n' frames    * Send_Packets --
 *sends out events from the OutList                     * Send_FrameSync -- Sends a FRAMESYNC packet *
 ...
 ** Build_Send_Packet -- Builds a big packet from a bunch of little ones.	* Add_Uncompressed_Events -- adds
 *uncompressed events to a packet       * Add_Compressed_Events -- adds compressed events to a packet        	*
 ...
 *                                                                         *
 * DoList Management:
 ** Execute_DoList -- Executes commands from the DoList                   * Clean_DoList -- Cleans out old events from
 *the DoList                 * Queue_Record -- Records the DoList to disk                            * Queue_Playback --
 *plays back queue entries from a record file         *
 *                                                                         *
 * Debugging:
 ** Compute_Game_CRC -- Computes a CRC value of the entire game.				* Add_CRC -- Adds a
 *value to a CRC                                      * Print_CRCs -- Prints a data file for finding Sync Bugs
 ** Init_Queue_Mono -- inits mono display                                 * Update_Queue_Mono -- updates mono display *
...
static RetcodeType Wait_For_Players(int first_time, ConnManClass *net, int resend_delta, int dialog_time, int timeout,
				    char *multi_packet_buf, int my_sent, long *their_frame, unsigned short *their_sent,
...
static RetcodeType Process_Receive_Packet(ConnManClass *net, char *multi_packet_buf, int id, int packetlen,
...
static int Can_Advance(ConnManClass *net, int max_ahead, long *their_frame, unsigned short *their_sent,
		       unsigned short *their_recv);
static int Process_Reconnect_Dialog(CDTimerClass<SystemTimerClass> *timeout_timer, long *their_frame, int num_conn,
...
// DoList management:
//...........................................................................
static int Execute_DoList(int max_houses, HousesType base_house, ConnManClass *net,
...
static void Print_Framesync_Values(long curframe, unsigned long max_ahead, int num_connections,
				   unsigned short *their_recv, unsigned short *their_sent, unsigned short my_sent);

extern void Keyboard_Process(KeyNumType &input);
void Dump_Packet_Too_Late_Stuff(EventClass *event, ConnManClass *net, long *their_frame, unsigned short *their_sent,
				unsigned short *their_recv);
...
static void Queue_AI_Multiplayer(void) {
	...
	if (Frame == 0 || Session.LoadGame) {
		...
		rc = Wait_For_Players(1, net, MIXFILE_RESEND_DELTA, FRAMESYNC_DLG_TIME * timeout_factor,
				      MIXFILE_TIMEOUT, multi_packet_buf, my_sent, their_frame, their_sent, their_recv);
	...
	rc = Wait_For_Players(
	    0, net, (Session.MaxAhead << 3), MAX(net->Response_Time() * 3, FRAMESYNC_DLG_TIME * timeout_factor),
	    iFramesyncTimeout * (2 * timeout_factor), multi_packet_buf, my_sent, their_frame, their_sent, their_recv);
#else
	rc = Wait_For_Players(
	    0, net, (Session.MaxAhead << 3), MAX(net->Response_Time() * 3, FRAMESYNC_DLG_TIME * timeout_factor),
	    FRAMESYNC_TIMEOUT * (2 * timeout_factor), multi_packet_buf, my_sent, their_frame, their_sent, their_recv);
#endif
 ...
 ** resend_delta			time (ticks) between FRAMESYNC resends				* dialog_time
 *time (ticks) until pop up a reconnect dialog		* timeout					time (ticks)
 *until we give up the ghost			* multi_packet_buf		buffer to store packets in
 ...
 ** RC_CANCEL				user hit 'Cancel' at the timeout countdown dlg	* RC_NOT_RESPONDING
 *other player(s) not responding						* RC_SCENARIO_MISMATCH	scenario's don't
 *match (first_time only)			* RC_DOLIST_FULL			DoList was full
 **
 *                                                                         *
 * WARNINGS:                                                               *
 *		none.
...
static RetcodeType Wait_For_Players(int first_time, ConnManClass *net, int resend_delta, int dialog_time, int timeout,
	...
	while (1) {
...
#endif
					if (WWMessageBox().Process(TXT_ASK_EMERGENCY_SAVE_NOT_RESPONDING, TXT_YES,
								   TXT_NO, TXT_NONE) == 0) {
		...
		while ((messages_this_loop++ < message_limit) &&
			...
			//------------------------------------------------------------------
			rc = Process_Receive_Packet(net, multi_packet_buf, id, packetlen, their_frame, their_sent,
						    their_recv);
			//..................................................................
		...
		// Debug output
		//---------------------------------------------------------------------
		Print_Framesync_Values(Frame, Session.MaxAhead, net->Num_Connections(), their_recv, their_sent,
				       my_sent);
		...
		if (Session.Am_I_Master()) {
			...
			if (Session.CommProtocol == COMM_PROTOCOL_MULTI_E_COMP) {
				ev.Data.FrameInfo.Delay =
				    max(((((resp_time / 8) + (Session.FrameSendRate - 1)) / Session.FrameSendRate) *
 ...
 * INPUT:                                                                  *
 *		net						ptr to connection manager
 ** multi_packet_buf		buffer to store packets in * multi_packet_max		max size of multi_packet_buf
 ** max_ahead				current game MaxAhead value * my_sent					#
 *commands I've sent this game						*
 *                                                                         *
 * OUTPUT:                                                                 *
...
static void Send_FrameSync(ConnManClass *net, int cmd_count) {
	...
	if (Session.CommProtocol == COMM_PROTOCOL_MULTI_E_COMP) {
		packet.Frame = ((Frame + Session.MaxAhead + (Session.FrameSendRate - 1)) / Session.FrameSendRate) *
			       Session.FrameSendRate;
	} else {
 ...
 * INPUT:                                                                  *
 *		net					ptr to connection manager
 ** multi_packet_buf	buffer containing packet(s) to parse					* id
 *id of sender													*
 ...
 ** their_sent or their_recv may have been 		* altered
 ** RC_PLAYER_READY:			player has been heard from for the 1st time; * this presumes that his original
 ** 'their_frame[]' value was -1 when this 		* routine was called
 ** RC_SCENARIO_MISMATCH:	FRAMEINFO scenario CRC doesn't match; 			* normally only applies after
 *loading a new 	* scenario or save-game								*
 ...
 ** RC_SERIAL_PROCESSED:	this was a SERIAL-type packet, and was 			* processed; the other player is
 *still connected,	* even if he's not in the game.							*
...
#endif
			Session.Messages.Add_Message(serial_packet->Name, serial_packet->ID,
 ...
 ** their_frame		array of their frame #'s * their_sent		array of their sent command count
 ...
 ** num_conn			# connections in 'their_frame' * reconn			1 = reconnect, 0 = waiting for
 *first-time connection	* fresh				1 = draw from scratch, 0 = only update time counter	*
 *                                                                         *
 * OUTPUT:                                                                 *
 ...
 * INPUT:                                                                  *
 *		net					ptr to connection manager
 ** their_frame			array containing frame #'s of other players			* their_sent
 *array containing command count of other players		* their_recv			array containing #
 *recv'd cmds from other players	*
 *                                                                         *
 * OUTPUT:                                                                 *
 ...
 *   11/21/1995 BRR : Created.                                             *
 *=========================================================================*/
static int Handle_Timeout(ConnManClass *net, long *their_frame, unsigned short *their_sent,
			  unsigned short *their_recv) {
	int oldest_index; // index of person requiring a reconnect
	int i, j;
	int id;
 ...
 ** frame_delay		desired frame delay to attach to all outgoing packets	* num_cmds			value to
 *use for the CommandCount field					* cap				max # events to
 *send												*
 *                                                                         *
 * OUTPUT:                                                                 *
 *		new size of packet
...
static int Build_Send_Packet(void *buf, int bufsize, int frame_delay, int num_cmds, int cap) {
	...
	//........................................................................
	if (Session.CommProtocol == COMM_PROTOCOL_MULTI_E_COMP) {
		finfo->Frame = ((Frame + frame_delay + (Session.FrameSendRate - 1)) / Session.FrameSendRate) *
			       Session.FrameSendRate;
	} else {
 ...
 *		buf				buffer to store packet in
 ** bufsize			max size of buffer
 ** frame_delay		desired frame delay to attach to all outgoing packets	* size				current
 *packet size												* cap
 ...
 *		buf				buffer to store packet in
 ** bufsize			max size of buffer
 ** frame_delay		desired frame delay to attach to all outgoing packets	* size
 *reference to current packet size								* cap
 *max # events to process											*
 *                                                                         *
 * OUTPUT:                                                                 *
 *		new size value
...
static int Add_Compressed_Events(void *buf, int bufsize, int frame_delay, int size, int cap) {
	...
	while (OutList.Count && (num < cap)) {
...
#if (0) // PG
					if (Debug_Print_Events) {
						printf("      adding Whom:%x (%x) Mission:%s Target:%x (%x) Dest:%x "
						       "(%x)\n",
		...
		if (Session.CommProtocol == COMM_PROTOCOL_MULTI_E_COMP) {
			OutList.First().Frame =
			    ((Frame + frame_delay + (Session.FrameSendRate - 1)) / Session.FrameSendRate) *
			    Session.FrameSendRate;
		} else {
			OutList.First().Frame = Frame + frame_delay;
		}
...
static int Extract_Compressed_Events(void *buf, int bufsize) {
	...
	while (leftover >= (datasize + (int)sizeof(EventClass::EventType))) {
		...
		if (event->Type != EventClass::FRAMESYNC) {
			...
			case (EventClass::MEGAMISSION):
				memcpy(&eventdata.Data.MegaMission, ((char *)buf) + pos + sizeof(EventClass::EventType),
				       datasize);
 ...
 ** base_house	HousesType to start with * net			ptr to connection manager; NULL if none
 ** skip_crc		a frame-based countdown timer; if it's non-zero, the		* CRC check will be skipped.
 *Ignored if NULL.					* their_frame	array of their frame #'s
 ** their_sent	array of # commands they've sent * their_recv	array of # commands I've received from them
 **
 *                                                                         *
 ...
 *=========================================================================*/
static int Execute_DoList(int max_houses, HousesType base_house, ConnManClass *net,
			  CDTimerClass<FrameTimerClass> *skip_crc, long *their_frame, unsigned short *their_sent,
			  unsigned short *their_recv) {
	HousesType house;
	HouseClass *hptr;
	int i, j, k;
					...
					if (Debug_Print_Events) {
						if (DoList[j].Type == EventClass::EXIT) {
							printf("(%d) Executing EXIT, ID:%d (%s), EvFrame:%d\n", Frame,
							       DoList[j].ID,
							       (HouseClass::As_Pointer((HousesType)(DoList[j].ID)))
								   ->IniName,
							       DoList[j].Frame);
						}
					}
							...
							if (WWMessageBox().Process(TXT_OUT_OF_SYNC, TXT_CONTINUE,
								...
								if (Session.Type == GAME_MODEM ||
									...
									while (net->Num_Connections()) {
										Keyboard->Check();
										Destroy_Connection(
										    net->Connection_ID(0), -1);
									}
								}
#if (TEN)
								else if (Session.Type == GAME_TEN && net) {
									while (net->Num_Connections()) {
										Destroy_TEN_Connection(
										    net->Connection_ID(0), -1);
									}
								}
#endif
	...
	for (house = HOUSE_MULTI1; house <= HOUSE_MULTI8; house++) {
		...
		if (housep) {
			...
			fprintf(fp, "%s: IsHuman:%d  Color:%s  ID:%d  ActLike:%s\n", housep->IniName, housep->IsHuman,
				ColorNames[color], housep->ID, HouseClass::As_Pointer(actlike)->Class->Name());
			Add_CRC(&GameCRC, (int)housep->Credits + (int)housep->Power + (int)housep->Drain);
			...
			for (i = 0; i < Units.Count(); i++) {
				...
				if (unitp->Owner() == house) {
					...
					fprintf(fp,
						...
						unitp->Coord, (int)unitp->PrimaryFacing, (int)unitp->SecondaryFacing,
			...
			for (i = 0; i < Vessels.Count(); i++) {
				...
				if (vesselp->Owner() == house) {
					...
					fprintf(fp,
						...
						vesselp->Coord, (int)vesselp->PrimaryFacing, vesselp->Get_Mission(),
						vesselp->Strength, vesselp->Class->Type, vesselp->As_Target());
				}
			}
	...
	if (net->Num_Connections() > 0) {
		...
		if (NetMonoMode == 0) {
			if (Frame == 0 || Session.LoadGame || NewMonoMode) {
				net->Configure_Debug(0, sizeof(CommHeaderType), sizeof(EventClass::EventType),
 ...
 * INPUT:                                                                  *
 *		net				ptr to connection manager
 ** flow_index		index # for flow-count updates								* -1:
 *display *
 *                                                                         *
 * OUTPUT:                                                                 *
 *		none.
 ...
 ** their_recv				# commands I've received from my connections		* their_sent
 *# commands each connection claims to have sent	* my_sent					# commands I've
 *sent										*
 *                                                                         *
 * OUTPUT:                                                                 *
 *		none.
	...
	if (net) {
		...
		for (i = 0; i < net->Num_Connections(); i++) {
			...
			fprintf(fp, "%12s  %2d    %6d      %6d      %6d\n", (HouseClass::As_Pointer(house))->IniName,
				net->Connection_ID(i), their_frame[i], their_sent[i], their_recv[i]);
		}
	}
...
void Check_Mirror(void) {
	...
	for (i = 0; i < DoList.Count; i++) {
		if (memcmp(&DoList[i], &MirrorList[i], sizeof(EventClass)) != 0) {
			sprintf(txt, "Queue Memory Trashed!  Head:%d Tail:%d, Addr:%p or %p", DoList.Get_Head(),
				DoList.Get_Tail(), DoList.Get_Array() + i, MirrorList.Get_Array() + i);
			WWMessageBox().Process(txt);
