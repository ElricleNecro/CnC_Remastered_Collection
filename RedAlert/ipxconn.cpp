 ...
 ** magicnum			the packet "magic number" for this connection			* address
 *address of destination (NULL = no address)				* id
 *connection's unique numerical ID								* name
 *connection's name * extralen			max size of app-specific extra bytes (optional)			*
 *                                                                         *
 * OUTPUT:                                                                 *
 *		none.
 **
 ...
 ** conn_num				local IPX Connection Number (0 = not logged in)		* listen_ecb
 *ptr to ECBType for listening								* send_ecb
 *ptr to ECBType for sending									* listen_header
 *ptr to IPXHeaderType for listening						* send_header			ptr to
 *IPXHeaderType for sending							* listen_buf			ptr to
 *buffer for listening								* send_buf
 *ptr to buffer for sending									* handler_rm_ptr
 *REAL-MODE pointer to event service routine			* (high word = segment, low word = offset)
 ...
 * prevented.
 ** Also, if the address of this IPX connection is known when the 				* constructor is called,
 *and Configure has been called, Get_Local_Target 	* is called to precompute the ImmediateAddress; this case is
 *detected & 	* if the value is already computed, it's just memcpy'd over.					*
 *                                                                         *
 * INPUT:                                                                  *
 *		buf			buffer to send
