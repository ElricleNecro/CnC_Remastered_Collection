...
int WOL_Chat_Dialog(WolapiObject *pWO) {
	...
	//------------------------------------------------------------------------
	typedef enum { REDRAW_NONE = 0, REDRAW_BACKGROUND, REDRAW_ALL = REDRAW_BACKGROUND } RedrawType;
...
//------------------------------------------------------------------------
	...
	while (process) {
		...
		if (::timeGetTime() > pWO->dwTimeNextWolapiPump) {
			/*
						if( pToolTipHitLast && pToolTipHitLast->bShowing )	//	Lame
			   hack. Problem is draws that occur in callbacks.
						{
		...
		*/
		...
		//	This hack, used elsewhere in this form, appears to be the standard dodge around
		// GadgetClass::Input's 	tendency to remove any focus the first time it runs for a 'commands'
		// list.

		//	ajw - Perhaps I could try doing this every cycle regardless - would avoid stupid non-focused
		// editbox key reactions bug.
		if (bHackFocus) {
			sendedit.Set_Focus();
		...
		case (BUTTON_JOIN | KN_BUTTON):
			//	Pressing the join button is exactly like doubleclicking on the selected index in
			// chanlist, except: 		if the first item is selected, ignore, unless we are at the top
			// level
		...
		case (BUTTON_LEAVE | KN_BUTTON):
			//	Because of the way things are set up, this is exactly like selecting the first item in
			// chanlist. 	(Button is disabled when this is not appropriate.)
		...
		case (BUTTON_BACK | KN_BUTTON):
			//	Pressing the back button is exactly like doubleclicking on the top item in chanlist,
			// except 	when we're at the top level.
...
bool EnterChannel(WolapiObject *pWO, IconListClass &chatlist, Channel *pChannel, char *szChannelName, bool bGame) {
		...
		if (pWO->CurrentLevel == WOL_LEVEL_INCHATCHANNEL || pWO->CurrentLevel == WOL_LEVEL_INLOBBY)
			if (!pWO->ExitChatChannelForGameChannel()) {
				*pWO->szChannelReturnOnGameEnterFail = 0;
				//				debugprint( "ExitChatChannelForGameChannel on join
				// failed" );
				pWO->bSelfDestruct = true;
				return false;
			}
	...
	/*	The following doesn't work because the needpw field is not currently being set in wolapi for chat
	   channels. Instead, we wait for a fail on join, then present this dialog and try again. if( pChannel->needpw )
		{
	...
	*szSuccessfulPassword = 0;
	...
	if (!bGame) {
		...
		if (hRes == S_OK) {
			*pWO->szChannelReturnOnGameEnterFail = 0;
			//	Return later to the lobby of the channel creator - which was saved in the channel
			// itself.
...
bool ProcessChannelListSelection(WolapiObject *pWO, IconListClass &chatlist, IconListClass &chanlist, int iIndex) {
	...
	if (szChannelType) {
		...
		if (strcmp(szChannelType, CHANNELTYPE_OFFICIALCHAT) == 0) {
			...
			//			debugprint( "Item selected in channel list unidentifiable from extradata
			// field\n" );
			pWO->bSelfDestruct = true;
			return false;
		}
