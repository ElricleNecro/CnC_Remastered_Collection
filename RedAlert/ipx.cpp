 ...
 *                    File Name : IPX.CPP                                  *
 *                                                                         *
 *                   Programmer : Barry Nance * from Client/Server LAN Programming			* Westwood-ized
 *by Bill Randolph				*
 *                                                                         *
 ...
 *                                                                         *
 * INPUT:                                                                  *
 *		connection_number		Connection Number to translate * network_number			ptr:
 *will hold Network Number							* physical_node			ptr:
 *will hold Node Address							*
 *                                                                         *
 * OUTPUT:                                                                 *
 *		0 = OK, -1 = error
 **
 ...
 ** Event_Service_Routine:	optionally points to a callback routine		* PacketCount:
 *set to 2, to tell IPX there are 2 areas to	* store the incoming data in
 ** Packet[0].Address:		set to the address of an IPXHeaderType			* Packet[0].Length:
 *sizeof(IPXHeaderType)								* Packet[1].Address:		address
 *of data buffer, for the packet			* Packet[1].Length:			size of the data buffer
 **
 *                                                                         *
 ...
 *										and are sending on
 ** Event_Service_Routine:	optionally points to a callback routine		* PacketCount:
 *set to 2, to tell IPX there are 2 areas the	* outgoing data is stored in
 ** Packet[0].Address:		set to the address of an IPXHeaderType			* Packet[0].Length:
 *sizeof(IPXHeaderType)								* Packet[1].Address:		address
 *of buffer containing data to send		* Packet[1].Length:			size of the data buffer
 ** ImmediateAddress:			must be filled in with the node address of	* the bridge that will route the
 *message;		* fill this in by calling IPX_Get_Local_Target	*
 *                                                                         *
 * Also, you must fill in the IPXHeaderType with certain values:				*
 *		PacketType:					set to 4 for IPX
 ** DestNetworkNumber:		Network Number of the destination system		* DestNetworkNode:
 *Node Address of the destination system			* DestNetworkSocket:		the destination system's
 *socket to send to;	* this doesn't have to be the same as the		* socket opened on the local machine.
 **
 *                                                                         *
