	...
	*/
	...
	if (ReadSavedNicks(pWO, NickList, szNameBuffer, szPassBuffer)) {
		...
		if (pWO->DoWebRegistration()) {
			//	User chose to go to web page. Leave function so that we'll re-read nicks when they
			// return.
			return 0;
		}
	}
...
void DeleteNick(WolapiObject *pWO, int iOneBasedEntryToDelete) {
	...
	//	If nick to delete is in position one, and there is a second nick, move the second nick into position
	// one.
...
); if( hFile == INVALID_HANDLE_VALUE ) return NULL; DWORD dwFileSize = GetFileSize( hFile, NULL ); char* pShp = new
char[ dwFileSize ]; DWORD dwBytesRead; ReadFile( hFile, pShp, dwFileSize, &dwBytesRead, NULL );
