...
#ifdef WOLAPI_INTEGRATION
			if (pWolapi)
				//	Just in case user was responding to a page from outside the game, and we had
				// frozen the "szExternalPager".
		...
		if (pWolapi->bInGame) {
			if (!pWolapi->bConnectionDown && ::timeGetTime() > pWolapi->dwTimeNextWolapiPump) {
				...
				if (pWolapi->bConnectionDown) {
					...
					//	ajw (Wolapi object is now left around, so we can try to send game
					// results.)
					//					//	Kill wolapi.
			...
			//	When showing a modal dialog during chat, this pumping is turned on. It's turned off
...
bool Force_CD_Available(int cd_desired) //	ajw
{
	...
	*/
	...
	if (Using_DVD()) {
		...
		cd_desired = CD_DVD;
		//		if( RequiredCD != -1 )
		//			RequiredCD = CD_DVD;		//	Just seems like a good idea. Not sure if
		// necessary.	ajw
	}
			...
			for (int i = 0; i < CDList.Get_Number_Of_Drives(); i++) {
				...
				if (cd_current >= 0) {
					...
					*/
					...
					if (cd_desired == cd_current || cd_desired == CD_ANY) {
						/*
						** Woohoo! The disk was in a different cd drive. Refresh the search path
						*list and return.
						*/
	...
	*/
	...
	//	Reason: This was triggering before Init_Secondary_Mixfiles(), which was screwing up the mixfile system
	// somehow.
	//
	//	Since the DVD is the only disk that can possibly be required when Using_DVD(), I never have to reload
	// the mix 	files here, because no other disk could ever have been asked for. And if not Using_DVD(),
