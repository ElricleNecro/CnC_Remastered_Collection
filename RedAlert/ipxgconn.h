 ...
 * overloaded:
 ** Send_Packet:		must embed the product ID into the packet header	* Receive_Packet:	must
 *detect resends via the LastAddress & 			* LastPacketID arrays. This class doesn't ACK * packets until
 *Service_Receive_Queue is called; 		* the parent classes ACK the packet when it's 			*
 *								received.
 ** Get_Packet:			extracts the product ID from the header;				* doesn't care
 *about reading packets in order			* Send					is capable of
 *broadcasting the packet, or sending	* to a specific address
 **
 *																									*
