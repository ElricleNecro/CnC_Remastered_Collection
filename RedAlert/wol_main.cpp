...
int WOL_Main() {
	...
	if (pWolapi) {
		...
		if (pWolapi->bConnectionDown) {
			// debugprint( "Re-entering WOL_Main(), pWolapi->bConnectionDown is true. Deleting old
			// WolapiObject...\n" );
	...
	if (!pWolapi) {
		...
		if (!pWolapi->bSetupCOMStuff()) {
			...
			//	We first assume that their wolapi.dll failed to register during wolsetup.exe, part of
			// the patch process. 	This happens if they have an outdated oleaut32.dll, such as the one that
			// comes with original 	version of Windows 95.

			//			debugprint( "bSetupCOMStuff failed. Attemping to reregister
		...
		while (bKeepGoing) {
			...
			if (bKeepGoing) {
				...
				case RESULT_WOLGSUP_FATALERROR:
					//					debugprint( "RESULT_WOLGSUP_FATALERROR
					// from game setup dialog.\n" ); 					Fatal(
...
bool ReregisterWolapiDLL() {
	...
	if (::RegOpenKeyEx(HKEY_LOCAL_MACHINE, "Software\\Westwood\\WOLAPI", 0, KEY_READ, &hKey) == ERROR_SUCCESS) {
		...
		if (::RegQueryValueEx(hKey, "InstallPath", 0, NULL, (LPBYTE)szInstallPath, &dwBufSize) ==
			...
			//	Something about Neal's extra threads only getting half-way set up before they get
			// deleted. 	(The extra threads shouldn't really be created in this case, anyway...)
			::Sleep(1000);
			FreeLibrary(hLib);
			FindClose(handle);
		} else {
...
void HandleDLLFail() {
	...
	//	We show either "call tech support" or "download IE3", depending on whether oleaut32.dll looks out of
	// date.
