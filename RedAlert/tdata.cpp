...
TerrainTypeClass::TerrainTypeClass(TerrainType terrain,
				   ...
				   short const *occupy,
				   short const *overlap)
	: ObjectTypeClass(RTTI_TERRAINTYPE, int(terrain), true, true, false, false, true, is_immune, true, fullname, ininame), Type(terrain),
	  CenterBase(centerbase), Theater(theater), IsWaterBased(is_water), Occupy(occupy), Overlap(overlap) {
	MaxStrength = 800;
	Armor = ARMOR_WOOD;
}
