	...
	if (hptr != NULL) {
		...
		case TEVENT_LEAVES_MAP:
			for (index = 0; index < Teams.Count(); index++) {
				...
				if (ptr->Class == Team && ptr->Is_Empty() && ptr->IsLeaveMap) {
					//					if (ptr->Class == Team && ptr->House ==
					// hptr && ptr->Is_Empty() && ptr->IsLeaveMap) {
					td.IsTripped = true;
					break;
				}
			}
		...
		*/
		case TEVENT_FAKES_DESTROYED:
			...
			// Indeed, STRUCTF_FAKEWEAP and STRUCTF_FAKECONST don't fit in the BScan bit range. ST -
			// 5/9/2019 if (hptr->BScan & (STRUCTF_FAKECONST|STRUCTF_FAKEWEAP)) return(false);
			break;
