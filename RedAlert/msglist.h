 ...
 *                                                                         *
 * Initializing:
 ** Call Init(), giving it the coords of the upper-left corner to display	* the messages, the coordinates to
 *display the editable message, max # 	* messages allowed, max # chars per message, and the pixel height of 	* the
 *font.  MaxChars should be less than or equal to the define 			* MAX_MESSAGE_LENGTH.
 **
 * 																								*
 *	Displaying a message:
 ** Call Add_Message(); the buffer you pass in is copied into a static		* buffer in this class.  Each message is
 *given a timeout; after this		* time, the message is removed from the displayed list.
 ** Each message also has a 2-byte ID, which can be used to identify the sender.
 *																									*
 * Editing a message:
 ** Call Add_Edit(), giving it the color & style to print in.  An 			* optional "To xxx" prefix is
 *allowed.  The edit message can appear in 	* the message list itself (it's placed first), or at a separate screen *
 *   location, depending on the values passed to Init().							*
 *																									*
 *	Updating messages:
 ** The Input() routine passes new keys to the editable message; this 		* routine's return code tells the app
 *whether to redraw the messages,	* and whether to send it the edit message.  As the user types
 ** characters into the edit field, if he overflows the max storage			* for the edit field, the field
 *is parsed & the first few words 			* removed; these words are put into the Overflow buffer, and the
 *app		* is told so; the app should send this buffer across to the destination.*
 *																									*
 ...
 ** When a message's timeout expires, it's deleted.  When a new message		* is added, the top message is deleted
 *if MPlayerMaxMessages is exceeded.	*
 *                                                                         *
