 ...
 *   NullModemClass::Init -- initialization * NullModemClass::Send_Message -- sends a message
 ...
 *                                                                         *
 * INPUT:                                                                  *
 *		numsend			# desired entries for the send queue * numreceive		# desired
 *entries for the receive queue					* maxlen			application's max packet
 *length								* magicnum
 *application-specific magic # (so we don't 				* accidentally end up talking to another one of
 *our own	* products using the same protocol)							*
 *                                                                         *
 * OUTPUT:                                                                 *
 *		none.
 **
 ...
 *=========================================================================*/
NullModemClass::NullModemClass(int numsend, int numreceive, int maxlen, unsigned short magicnum) : ConnManClass() {
	/*------------------------------------------------------------------------
 ...
 *		port				address
 ** irq				2-15
 ** dev_name			name of communications device (win32 only)           * baud
 *300, 1200, 9600, etc											  * parity
 *'O' (odd), 'E' (even), 'N' (none), 'S' (space),		  * 'M' (mark)
 ...
 * INPUT:                                                                  *
 *		index			(not used)
 ** type_offset		ID's byte offset into packet * type_size		size of ID, in bytes; 0 if none
 ** names				ptr to array of names; use ID as an index into this	* namestart
 *numerical start of the 1st name value						* namecount		# in the names
 *array; 0 if none.								*
 *                                                                         *
 * OUTPUT:                                                                 *
 *		none.
