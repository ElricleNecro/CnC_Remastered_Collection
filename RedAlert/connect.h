 ...
 *                                                                         *
 * DESCRIPTION:
 ** This class represents a single "connection" with another system.  It's	* a pure virtual base class that acts as
 *a framework for other classes.	*
 *																									*
 ...
 *                                                                         *
 * THE PACKET HEADER:
 ** The Connection Classes prefix every packet sent with a header that's		* local to this class.  The
 *header contains a "Magic Number" which should	* be unique for each product, and Packet "Code", which will tell the
 ** receiving end if this is DATA, or an ACK packet, and a packet ID, which	* is a unique numerical ID for this
 *packet (useful for detecting resends).* The header is stored with each packet in the send & receive Queues; * it's
 *removed before it's passed back to the application, via				* Get_Packet()
 **
 *                                                                         *
 * THE CONNECTION MANAGER:
 ** It is assumed that there will be a "Connection Manager" class which 		* will handle parsing incoming
 *packets; it will then tell the connection	* that new packets have come in, and the connection will process them in
 ** whatever way it needs to for its protocol (check for resends, handle		* ACK packets, etc).  The job of
 *the connection manager is to parse			* incoming packets & distribute them to the connections that
 *need to		* store them (for multi-connection protocols).
 **
 *                                                                         *
 * NOTES ON ACK/RETRY:
 ** This class provides a "non-sequenced" ACK/Retry approach to packet		* transmission.  It sends out as many
 *packets as are in the queue, whose	* resend delta times have expired; and it ACK's any packets its received
 ** who haven't been ACK'd yet.  Thus, order of delivery is NOT guaranteed;	* but, the performance is better than a
 *"sequenced" approach.  Also, the	* Packet ID scheme (see below) ensures that the application will read * the
