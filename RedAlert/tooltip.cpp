...
void ToolTipClass::Show() {
	if (!bShowing) {
		...
		//	Draw text.
		// Simple_Text_Print( szTipUse, xShowUse, yShowUse, GadgetClass::Get_Color_Scheme(), ColorRemaps[
		// PCOLOR_BROWN ].Color, TPF_TYPE ); //TPF_DROPSHADOW );
		Simple_Text_Print(szTipUse, xShowUse, yShowUse, GadgetClass::Get_Color_Scheme(), BLACK,
				  TPF_TYPE); // TPF_DROPSHADOW );
		//	Draw bounding rect.
		//		LogicPage->Draw_Rect( xShowUse, yShowUse, xShowUse + wShowUse - 1, yShowUse + hShow - 1,
		// ColorRemaps[ PCOLOR_GOLD ].Color );
...
void ToolTipClass::Unshow() {
	if (bShowing) {
		...
		if (!bIconList) {
			...
			//				return;
			//			}
			//			Fancy_Text_Print( TXT_NONE, 0, 0, TBLACK, TBLACK, TPF_TYPE );	//
			// Required before String_Pixel_Width() call, for god's sake. 			wShowUse =
			// String_Pixel_Width( szTipUsed ) + 2; 			if( bRightAlign )
			// xShowUse -= wShowUse;
			xShowUse = xLastShow;
			yShowUse = yLastShow;
			wShowUse = wLastShow;
...
bool ToolTipClass::bOverDifferentLine() const {
	...
	//	Returns true if the iconlist line that the mouse is over is different than the last time Show() was
	// called.
