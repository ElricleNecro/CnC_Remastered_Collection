 ...
 ** NET_QUERY_PLAYER
 ** Name:					name of game we want players to respond for * NET_ANSWER_PLAYER
 ** Name:					player's name
 ** PlayerInfo:			info about player
 ** NET_CHAT_ANNOUNCE
 ** Chat:					unique id of the local node, so I can tell * if this chat announcement
 *is from myself					  * NET_QUERY_JOIN
 ** Name:					name of player wanting to join
 ** PlayerInfo:			player's requested house, color, & version range		  * NET_CONFIRM_JOIN
 ...
 * 																														  *
 *---------------------------------------------------------------------------------------------*
 * Functions: * Init_Network -- initializes network stuff * Shutdown_Network -- shuts down network stuff
 ** Process_Global_Packet -- responds to remote queries * Destroy_Connection -- destroys the given connection
 ** Remote_Connect -- handles connecting this user to others * Net_Join_Dialog -- lets user join an existing game, or
 *start a new one						  * Request_To_Join -- Sends a JOIN request packet to
 *game owner           	 					  * Unjoin_Game -- Cancels joining a game *
 ...
 ** Get_NewGame_Responses -- processes packets for New Game dialog                            * Compute_Name_CRC --
 *computes CRC from char string                                         * Net_Reconnect_Dialog -- Draws/updates the
 *network reconnect dialog                        *
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
 ...
 * and NET_QUERY_PLAYER.  The other commands are too context-specific to be able * to handle here, such as joining the
 *game or signing off; but this routine handles			  * the majority of the program's needs.
 **
 *                                                                         						  *
 ...
 **
 *                                                                         						  *
 * WARNINGS: * Session.GameName must have been filled in before this function can be called. *
 *                                                                         						  *
 ...
 *                                                                         						  *
 * INPUT: * id			connection ID to destroy; this should be the HousesType of the player * on this
 *connection
		...
		*/
		...
		if (display) {
			...
			if (display >= REDRAW_PARMS && parms_received && joinstate >= JOIN_CONFIRMED) {
				...
				// Scenario title
				//...............................................................
				//				LogicPage->Fill_Rect(d_dialog_x + 16 *RESFACTOR,
				// d_name_y, d_dialog_x + d_dialog_w - 16 *RESFACTOR, d_name_y + d_txt6_h, BLACK);
				...
				if (Session.Options.ScenarioDescription[0]) {
					...
					// descrip.Set_Text(txt); 					Fancy_Text_Print("%s %s", d_dialog_cx, d_name_y,
					// scheme, BLACK, TPF_TEXT | TPF_CENTER, p,
					// Session.Options.ScenarioDescription);
				} else {
					sprintf(txt, "%s %s", p, Text_String(TXT_NOT_FOUND));
					descrip.Set_Text(txt);
					//					Fancy_Text_Print("%s %s", d_dialog_cx,
					// d_name_y, &ColorRemaps[PCOLOR_RED], TBLACK, TPF_TEXT | TPF_CENTER, p,
				...
				//				Fancy_Text_Print(txt, d_aiplayers_x + d_aiplayers_w + 2
				//*RESFACTOR, d_aiplayers_y, scheme, BLACK, TPF_TEXT);
			}
 ...
 ** join_index		index of game we're joining * house				requested house
 ...
 ** playerlist		ListBox of player names * game_index		index in 'gamelist' of game we're leaving
 ** goto_lobby		true = we're going to the lobby								* msg_x
 *message system x-coord											* msg_y
 *message system y-coord											* msg_h
 *message system char height										* send_x
 *message system send x-coord									* send_y
 *message system send y-coord									* msg_len
 *message system max msg length									*
 *                                                                         *
 * OUTPUT:                                                                 *
 *                                                                         *
 * WARNINGS:                                                               *
 ...
 ** playernow	if 1, will immediately send the player query for currently-selected game	  * chatnow
 *if 1, will immediately send the chat announcement * myname		user's name
		...
		*/
		...
		if (display) {
			...
			if (display >= REDRAW_PARMS) {
				//				LogicPage->Fill_Rect(d_count_x + d_count_w +
				// 2*RESFACTOR, d_count_y, d_count_x + d_count_w + 35*RESFACTOR, d_aiplayers_y +
				// d_aiplayers_h+RESFACTOR, BLACK);

				sprintf(txt, "%d", Session.Options.UnitCount);
				staticunit.Set_Text(txt);
				staticunit.Draw_Me();
				//				Fancy_Text_Print(txt, d_count_x + d_count_w +
				// 2*RESFACTOR, d_count_y, scheme, BLACK, TPF_TEXT);
				...
				staticlevel.Set_Text(txt);
				staticlevel.Draw_Me();
				//				Fancy_Text_Print(txt, d_level_x + d_level_w +
				// 2*RESFACTOR, d_level_y, scheme, BLACK, TPF_TEXT);

				sprintf(txt, "%d", Session.Options.Credits);
				staticcredits.Set_Text(txt);
				staticcredits.Draw_Me();
				//				Fancy_Text_Print(txt, d_credits_x + d_credits_w +
				// 2*RESFACTOR, d_credits_y, scheme, BLACK, TPF_TEXT);

				sprintf(txt, "%d", Session.Options.AIPlayers);
				staticaiplayers.Set_Text(txt);
				staticaiplayers.Draw_Me();
				//				Fancy_Text_Print(txt, d_aiplayers_x + d_aiplayers_w +
				// 2*RESFACTOR, d_aiplayers_y, scheme, BLACK, TPF_TEXT);
			}
		...
		case (BUTTON_OK | KN_BUTTON):
			...
			if (Session.Players.Count() > 1) {
				//				if (Session.Players.Count() + Session.Options.AIPlayers
				//> 1 ) {
				rc = TRUE;
				process = FALSE;
			} else {
 ...
 * OUTPUT:                                                                 *
 *		EV_NONE = nothing happened
 ** EV_NEW_PLAYER = a new player has joined; false otherwise					* EV_PLAYER_SIGNOFF = a
 *player has left											* EV_MESSAGE = a
	...
	if (Process_Global_Packet(&Session.GPacket, &Session.GAddress) != 0) {
		...
		*/
		...
		//.....................................................................
			...
			//..................................................................
#ifdef FIXIT_VERSION_3
			//	Added to the transmitted _min_ version number is a bit indicating presence of Aftermath
			// expansion.
			...
			if (bGuestHasAftermath)
				...
				//				debugprint( "Host hears guest say 'I don't have
				// Aftermath'\n" );
				if (bAftermathMultiplayer) {
					//					debugprint( "Host decides this is no
					// longer an Aftermath game!\n" );
		...
		if (event == EV_STATE_CHANGE) {
			...
			if (joinstate == JOIN_GAME_START || joinstate == JOIN_GAME_START_LOAD) {
				...
				if (!load_game) {
						...
						if (!check_file.Is_Available()) {
							...
							if (Get_CD_Index(current_drive, 1 * 60) != 2) {
								...
								*/
								...
								/*
								** See if that scenario is available now. Its pretty
								*fatal if it isnt.
								*/
								Session.Options.ScenarioIndex = -1;
								for (i = 0; i < Session.Scenarios.Count(); i++) {
					...
					**	need to request that it is sent. If we can identify the scenario locally
...
#ifndef FIXIT_VERSION_3 //	Removed restriction on downloading official maps.
						/*
						** Oh dear. Thats a scenario I dont have. Request that the host sends
						*the *	scenario to me provided its not an official Westwood scenario.
						**
