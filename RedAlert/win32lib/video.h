 ...
 *																									*
 * Signature    - Will always be 'VESA' * Version      - Version #
 ** OEMString    - OEM ID string
 ** Capabilities - Not defined by VESA yet
 ** AvailModes   - List of available modes; terminated with -1 (0xffff)		* TotalMemory  - ???
...
/*=========================================================================*
 * VesaModeInfoType - Info about this VESA mode * (Filled in by VESA BIOS Function 1)
 **
 *																									*
 * Attributes       - bit 0: 1 = mode is supported * bit 1: 1 = optional info available
 ** bit 2: 1 = std BIOS output funcs valid in this mode	* bit 3: 0 = monochrome, 1 = color
 ** bit 4: 0 = text mode, 1 = graphics							* WinA_Attributes  - bit 0 = win
 *exists, bit 1=readable, bit 2= writable	* WinB_Attributes  - bit 0 = win exists, bit 1=readable, bit 2= writable
 ** WinGranularity   - smallest address boundary window can be placed upon;	* size is in KB (ie 64, 32, 4)
 ...
 * XRes             - X-resolution
 ** YRes             - Y-resolution
 ** XCharSize        - Horizontal size of char cell * YCharSize        - Vertical size of char cell
 ** NumPlanes        - # of memory planes (???) * BitsPerPixel     - # bites per pixel
 ** NumBanks         - # of banks (ie planes) * MemoryModel      - 00h = Text mode
 ** 01h = CGA mode * 02h = Hercules
 ** 03h = 4 plane planar mode										* 04h = packed
 *pixel mode (1 byte/pixel)					* 05h = non-chain 4, 256-color mode
 ** 06-0Fh = * 10-FFh = OEM-specific * BankSize         - Bank size in KB
 **
 *=========================================================================*/
#ifdef NOT_FOR_WIN95
typedef struct {
